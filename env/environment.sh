...
# Inicia o Open5GS no HOST2 em segundo plano
run_remote "$HOST2" "cd $srs_5gc && nohup docker compose up --build 5gc > /tmp/open5gs.log 2>&1 &" &
echo "Open5GS está executando."
sleep 5

# Inicia o ORAN-RIC-SC no HOST2 em segundo plano
run_remote "$HOST2" "cd $ric_sc_path && nohup docker compose up  > /tmp/oran-ric-sc.log 2>&1 &" &
echo "ORAN-RIC-SC está executando."

sleep 20  # Aguarda 5 segundos para garantir que o ORAN-RIC-SC esteja ativo
...
echo "srsRAN gNB está em execução." "$H1_pwrd"
sleep 5
